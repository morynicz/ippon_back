...
class TournamentAdminViewTest(APITestCase):
    def setUp(self):
        ...
        self.to1 = Tournament.objects.create(name='T1', webpage='http://w1.co', description='d1', city='c1',
                                             ...
                                             sex_constraint=1)
        self.a1 = TournamentAdmin.objects.create(user=self.u1, tournament=self.to1, is_master=True)
...
class TournamentAdminViewSetAuthorizedTests(TournamentAdminViewTest):
    ...
    def test_creates_admin_with_valid_payload(self):
        ...
        self.assertTrue(TournamentAdmin.objects.filter(tournament=self.to1.id, user=self.u2).exists())
...
class TournamentAdminViewSetUnauthorizedTests(TournamentAdminViewTest):
    ...
    def test_admin_creation_attempt_gets_forbidden(self):
        ...
        self.assertFalse(TournamentAdmin.objects.filter(tournament=self.to1.id, user=self.u2).exists())
