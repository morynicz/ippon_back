...
class TournamentViewTest(APITestCase):
    def setUp(self):
        ...
        self.to2 = tm.Tournament.objects.create(
            name="T2",
            webpage="http://w2.co",
            description="d2",
            city="c2",
...
class TournamentViewSetAuthorizedTests(TournamentViewTest):
    ...
    def test_delete_not_existing_tournament_returns_bad_request(self):
        response = self.client.delete(
            reverse("tournament-detail", kwargs={"pk": iuv.BAD_PK})
        )
...
class TournamentViewSetUnauthorizedTests(TournamentViewTest):
    ...
    def test_delete_gets_forbidden(self):
        response = self.client.delete(
            reverse("tournament-detail", kwargs={"pk": self.to1.id})
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class TournamentAdminTest(TournamentViewTest):
    def setUp(self):
        ...
        self.adm2 = tm.TournamentAdmin.objects.create(
            user=self.user2, tournament=self.to1, is_master=False
        )
...
class UnauthorizedTournamentAdminTest(TournamentAdminTest):
    ...
    def test_get_admins_returns_forbidden(self):
        response = self.client.get(
            reverse("tournament-admins", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_get_non_admins_returns_forbidden(self):
        response = self.client.get(
            reverse("tournament-non-admins", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class AuthorizedTournamentAdminTest(TournamentAdminTest):
    def setUp(self):
        ...
        self.adm1 = tm.TournamentAdmin.objects.create(
            user=self.user, tournament=self.to1, is_master=True
        )
        self.client.force_authenticate(user=self.user)
        ...
        expected = [
            {
                ...
                "user": {"id": self.user2.id, "username": "user2"},
            },
            {
                "id": self.adm1.id,
                "is_master": True,
    ...
    def test_get_admins_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-admins", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_get_non_admins_for_valid_fight_returns_list_of_tournaments_non_admins(
        self,
    ):
        expected = [
            {"id": self.user3.id, "username": "user3"},
            {"id": self.user4.id, "username": "user4"},
        ]
        response = self.client.get(
            reverse("tournament-non-admins", kwargs={"pk": self.to1.pk})
        )
    ...
    def test_get_non_admins_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-non-admins", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class UnauthenticatedParticipantsTest(TournamentParticipantsTest):
    ...
    def test_get_participants_returns_unauthorized(self):
        response = self.client.get(
            reverse("tournament-participants", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_get_non_participants_returns_unauthorized(self):
        response = self.client.get(
            reverse("tournament-non-participants", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_get_participations_returns_unauthorized(self):
        response = self.client.get(
            reverse("tournament-participations", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class AuthorizedParticipantsTest(TournamentParticipantsTest):
    ...
    def test_get_participants_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-participants", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_get_non_participants_for_valid_tournament_returns_list_of_tournaments_non_participants(
        self,
    ):
        expected = [
            {
    ...
    def test_get_non_participants_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-non-participants", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_get_participations_for_valid_tournament_returns_list_of_participations(
        self,
    ):
        expected = [
            {
                "player": {"id": self.p1.id, "name": "pn1", "surname": "ps1"},
                "id": self.par1.id,
                "is_paid": False,
                "is_registered": False,
                "is_qualified": True,
            ...
            {
                "player": {"id": self.p2.id, "name": "pn2", "surname": "ps2"},
                "id": self.par2.id,
                "is_paid": False,
                "is_registered": False,
                "is_qualified": False,
    ...
    def test_get_participations_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-participations", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TournamentTeamTests(TournamentViewTest):
    ...
    def test_get_teams_returns_list_of_tournament_teams(self):
        expected = [
            {"id": self.t1.id, "name": "t1", "members": [], "tournament": self.to1.id},
            {"id": self.t2.id, "name": "t2", "members": [], "tournament": self.to1.id},
        ]
        response = self.client.get(
            reverse("tournament-teams", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(expected, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_teams_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-teams", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TournamentAdminAuthorizationsTest(TournamentViewTest):
    ...
    def test_admin_auth_admin_returns_200_and_isAuthorized_true(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(
            reverse("tournament-admin-authorization", kwargs={"pk": self.to1.pk})
        )
    ...
    def test_admin_auth_staff_returns_200_and_isAuthorized_true(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(
            reverse("tournament-staff-authorization", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual({"isAuthorized": True}, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TournamentGroupPhasesUnauthenticatedTests(TournamentViewTest):
    ...
    def test_get_group_phases_for_valid_tournament_returns_list_of_group_phases(self):
        ...
        response = self.client.get(
            reverse("tournament-group_phases", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(expected, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_group_phases_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-group_phases", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TournamentCupPhasesUnauthenticatedTests(TournamentViewTest):
    ...
    def test_get_cup_phases_for_valid_tournament_returns_list_of_cup_phases(self):
        ...
        response = self.client.get(
            reverse("tournament-cup_phases", kwargs={"pk": self.to1.pk})
        )
        self.assertEqual(expected, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_cup_phases_for_invalid_tournament_returns_not_found(self):
        response = self.client.get(
            reverse("tournament-cup_phases", kwargs={"pk": iuv.BAD_PK})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TournamentUnassignedPlayersTests(TournamentViewTest):
    ...
    def test_calling_get_on_not_assigned_participants_when_not_authenticated_returns_unauthorised(
        self,
    ):
        response = self.client.get(
            reverse("tournament-not_assigned", kwargs={"pk": self.to1.id})
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_calling_get_on_not_assigned_participants_when_not_authorized_returns_forbidden(
        self,
    ):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(
    ...
    def test_calling_get_on_not_assigned_participants_when_admin_returns_list_of_unassigned(
        self,
    ):
        tm.TournamentAdmin.objects.create(
        ...
        self.assertEqual(
            response.data,
            [
                {"id": self.p3.id, "name": "pn3", "surname": "ps3"},
                {"id": self.p4.id, "name": "pn4", "surname": "ps4"},
            ],
        )

    def test_calling_get_on_not_assigned_participants_when_owner_returns_list_of_unassigned(
        self,
    ):
        tm.TournamentAdmin.objects.create(
            user=self.user, tournament=self.to1, is_master=True
    ...
    def test_calling_get_un_unassigned_participants_when_authorized_returns_only_qualified_players(
        self,
    ):
        tm.TournamentAdmin.objects.create(
            user=self.user, tournament=self.to1, is_master=False
