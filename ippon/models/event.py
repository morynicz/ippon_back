...
class Event(models.Model):
    ...
    def registration_is_open(self) -> bool:
        ...
        if (
            self.registration_start_time.replace(tzinfo=pytz.UTC)
            < now.replace(tzinfo=pytz.UTC)
            < self.registration_end_time.replace(tzinfo=pytz.UTC)
        ):
            return True
        else:
            return False
    ...
    def has_started(self) -> bool:
        now = datetime.datetime.now()
        if (
            self.start_time.replace(tzinfo=pytz.UTC)
            < now.replace(tzinfo=pytz.UTC)
            < self.end_time.replace(tzinfo=pytz.UTC)
        ):
            return True
        else:
            return False
